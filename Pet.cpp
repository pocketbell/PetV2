...
//Adjust Pet Values using Funct.h Functions
//MaxValue, MinValue, RaiseValue(+=), LowerValue (-=), SetValue(=)
...
//Auto Update Pet States based on set value range
...
//Output current Pet States as String
std::string Pet::StateToString(Pet::Value value)
{
	switch (value)
	{
	case Pet::Value::Mood:
		if		(m_moodState == Pet::Mood::Happy)				return "[Happy]";
		else if (m_moodState == Pet::Mood::Okay)				return "[Okay]";
		else if (m_moodState == Pet::Mood::Aggravated)			return "[Aggravated]";
		else if (m_moodState == Pet::Mood::Sad)					return "[Sad]";
		break;
	case Pet::Value::Hunger:		 
		if		(m_hungerState == Pet::Hunger::Full)			return "[Full]";
		else if (m_hungerState == Pet::Hunger::Fine)			return "[Fine]";
		else if (m_hungerState == Pet::Hunger::Hungry)			return "[Hungry]";
		else if (m_hungerState == Pet::Hunger::Starving)		return "[Starving]";
		break;
	case Pet::Value::Hygiene:
		if		(m_hygieneState == Pet::Hygiene::Clean)			return "[Clean]";
		else if (m_hygieneState == Pet::Hygiene::Normal)		return "[Normal]";
		else if (m_hygieneState == Pet::Hygiene::Dirty)			return "[Dirty]";
		else if (m_hygieneState == Pet::Hygiene::Filthy)		return "[Filthy]";
		break;
	}
	return "NoVal";
}
